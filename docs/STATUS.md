...
> Son güncelleme: 2026-05-30, **canonical Emlakjet filter enrichment Task 1 revizyonu DONE**: authoritative tam listeye göre kategori zinciri (`trade_type`, `property_category`, `property_type`), çoklu fiyat para birimi (`price_amount`, `price_currency`) ve eksik `Saten Boya` checkbox'ı eklendi. Registry artık `27` structured filtre + `124` checkbox seçeneği içeriyor. `price_tl` canonical registry dışında geçici uyumluluk output'u olarak kalacak. Registry testi: `4 passed`.
...
- **Canonical filter enrichment** — **AKTİF**: Task 1 revizyonu DONE. Sıradaki Task 2 revizyonu: scraper/cleaner kategori ve çoklu para birimi canonical alanlarını deterministic dolduracak.
