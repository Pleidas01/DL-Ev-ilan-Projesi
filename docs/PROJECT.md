...
### 7a. Katman A — `facts_gold` (22 alan)
...
**HYBRID (5 — features/title + description LLM + image VLM fallback):**
16. `kitchen_type` — features → desc LLM → image VLM; enum `amerikan_acik|kapali_ayri|yari_acik|null`
17. `has_balcony` — features → desc LLM → image VLM; bool|null
18. `has_elevator` — features "Asansör" → title regex → desc LLM; bool|null
19. `has_parking` — features → desc LLM; bool|null
20. `has_aircon` — features "Klima" → title/desc LLM → image VLM; bool|null

**DESCRIPTION-ONLY (2 — LLM extraction):**
21. `near_metro` — desc LLM, bool|null, `<15dk yürüme`
22. `near_metrobus` — desc LLM, bool|null

### 7b. Katman B — `visual_gold` (12 alan)

Primary VLM, fallback/cross-check `property_features`:
...
```json
{
  ...
  "soft_features": {
    ...
    "visual_gold": { "...": "12 visual_gold alan adları" }
  },
...
| Description extraction | per-field accuracy (8 hybrid+desc alanı) | ≥ 0.75 | `labeling/gold_listings_manual_todo.jsonl:facts_gold` |
| Image labeling | per-field Jaccard / accuracy (12 visual alan) | ≥ 0.70 | `labeling/gold_listings_manual_todo.jsonl:visual_gold` |
